...
export function UserPanel({ users }: props) {
  ...
  return (
    <div className='w-1/6 h-full bg-gray-200 flex flex-col overflow-hidden relative'>
      ...
      <div className='flex-1 py-4 flex flex-col gap-y-10 absolute top-0 bottom-0 left-0 -right-3 overflow-y-scroll mt-20'>
        {users.map((user) => (
          <UserCircle
            key={user.id}
      ...
      <div className='flex p-6 bg-gray-300 absolute bottom-0 w-64 box-content'>
        <form className='ml-12' action='/logout' method='post'>
          <button
            type='submit'
